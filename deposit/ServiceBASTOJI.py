...
_mainkNode = {"rpcuser": "rpc", "rpcpassword": "rpc", "rpcport": 2222, "rpcipaddress": "localhost"}
_networkNode = {"rpcuser": "rpc", "rpcpassword": "rpc", "rpcport": 4444, "rpcipaddress": "localhost"}
_aliceNode = {"rpcuser": "rpc", "rpcpassword": "rpc", "rpcport": 6666, "rpcipaddress": "localhost"}
_gastonNode = {"rpcuser": "rpc", "rpcpassword": "rpc", "rpcport": 8888, "rpcipaddress": "localhost"}

# list of all nodes
_listNodes = [_mainkNode,_networkNode,_aliceNode,_gastonNode]
    ...
    def call(self, rpcMethod, *params):
        ...
        while True:
            try:
                ...
                print(
                    "Couldn't connect for remote procedure call, will sleep for five seconds and then try "
                    "again ({} more tries)".format(
                        tries))
                time.sleep(10)
            else:
...
class ServiceBastoji():
    def __init__(self,receiverAddress, amountToSend):
        # transaction paramateres
        self._receiverAddress = receiverAddress
        self._amountToSend = amountToSend
        # dinamic variable
        ...
        self._session = requests.Session()
        self._rpcServerURL = 'http://' + self._rpcUser + ':' + self._rpcPassword + '@' + self._ipAddress + ':' + str(self._portNumber)
        self._headers = {'content-type': 'application/json'}
        # variable output
        ...
        self._transactionFixedFee = 0.00020000
        self._changeMinimumAmount = 0.00000001
        self._changeAmountToBeBack = 0.0
        # rpc functions
    ...
    the value at 2 actually indicates 2*1e8 = 2e8 = 2 * 10^8 = 200,000,000. (sawhiletoshi)
     """
    ...
    def calculateTransactionFeeAndReturnCahngeAmount(self):
        ...
        return float(format(self._changeAmountToBeBack, '.8f'))
    ...
    def createChangeAddress(self):
        return self.call(self._funcCreateNewAddress)

    """this function return a dict of spendable UTXOs"""
    def getUtxoInfos(self):
        nested_dict = {}

        for v in self.call(self._funcGetListUnspent):
    ...
    def selectedUtxos(self):
            ...
            while self._amountZero < self._amountToSend:
                ...
                for k, v in self._dictionary.items():
                    if self._dictionary[k]['amount'] == self.getMinAmountofUtxos():
                        ...
                        self._amountZero += self.getMinAmountofUtxos()
                        self._key = k
                    else:
                        continue
        ...
        #print(outputDict)
        #print("amountZero :" ,self._amountZero)
        return outputDict, self._amountZero
    ...
    def createRawTxFropmUtxo(self):
        ...
        if selectedDict:
            for k, v in selectedDict.items():
                hello = '''[{"txid": "%s", "vout": %d}]''' '''{"%s": %f, "%s": %f}''' \
                        % (k, v['vout'],self._receiverAddress,self._amountToSend,str(self.createChangeAddress()),self.calculateTransactionFeeAndReturnCahngeAmount())
                # print(k)
            ...
            print(hello)
            #print(paramsRawTx)
            playload = json.dumps({'jsonrpc': '2.0', "method": self._funcCreaterawtrasaction, "params": hello})
    ...
    def signRawTxWithWallet(self):
        return self.call(self._funcSignTransactionWithWallet, self.createRawTxFropmUtxo())['hex']
    ...
    def main(self):
        ...
        #print("this is hex transaction :D ",transactinHex)
        time.sleep(15)
...
print(serverRpcURL)
print(RPCHost(serverRpcURL).call(_funcGetListUnspent))"""


networkAddress='johHJXZ14xLodRF6VpozjwqBqnHfXj4a7X'
aliceaddress ='jssboB6gXSkWHuJwHmVKzehVhD9bzGLcuX'
gastonAddres = 'jrNDJCmdWxzsT6vaxoQKxLKYkpGwYMpaiL'
...
print(ServiceBastoji(receiverAddress, amountToSend).createRawTxFropmUtxo())
