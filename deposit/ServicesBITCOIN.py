...
class ServiceBitcoin(object):
    def __init__(self, receiverAddress, cryptoAmountToSend):
        ...
        self._transactionFixedFee = 0.0002
    ...
    def calculateTransactionFeeAndReturnCahngeAmount(self):
        ...
        if totalSelectedUtxoAmount > validAmount:
            self._changeAmountToBeBack = totalSelectedUtxoAmount - (
                        self._amountToSend + self._transactionFixedFee)
    ...
    def main(self):
        ...
        time.sleep(20)
        blockNumber = rpc_connection.generate(1)
        print('rrrrrrrrrrrrrrrrrrr')
        print('this the process i want to run : ',)

        time.sleep(20)
        if transactinHex not in self.checkMemoPoolForUncofrmedTx():
            return  {'status': 'Success', 'block': blockNumber}
        else:
...
#print("BTCprice: " ,ServiceBitcoin(receiverAddress='2N1ViVJJPWgN52BaAbbzQGw1LF5MVUMah76',cryptoAmountToSend=0.011).main())
